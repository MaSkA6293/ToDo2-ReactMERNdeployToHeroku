...
const Leftpanel = ({ list, colors, onActivItem }) => {
  ...
  return (
    <Fragment>
      <div
        className={classnames("leftpanel", isopem ? "leftpanel_openPanel" : "")}
      >
        <Lists
          items={[{ icon: iconList, name: "Все задачи" }]}
          allListTask={true}
          buttonMobyle
          toggleMobyleMenu={toggleMobyleMenu}
        />
        {list ? (
          <Lists items={list} btn onactivItem={onActivItem} />
        ) : (
          "Загрузка..."
        )}
        {list ? (
          <AddNewList
            colors={colors}
            items={[{ icon: iconPlus, name: "Добавить список!" }]}
          />
        ) : (
          "Загрузка Add..."
        )}
